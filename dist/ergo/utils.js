...
import { ErgoAddress, SByte, SColl, SGroupElement } from '@fleet-sdk/core';
...
export function parseLongColl(renderedValue) {
...
export function parseIntFromRendered(renderedValue) {
...
export function parseIntFromHex(renderedValue) {
...
export function utf8StringToCollByteHex(inputString) {
    const bytes = stringToBytes('utf8', inputString);
    return SColl(SByte, bytes).toHex();
}
export function bigintToLongByteArray(value) {
 ...
 */
export function serializedToRendered(serializedValue) {
 ...
 */
export function stringToRendered(value) {
    return serializedToRendered(SString(value));
}
export function pkHexToBase58Address(pkHex) {
    if (!pkHex)
        return "N/A";
    try {
